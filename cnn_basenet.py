...
class CNNBaseModel(object):
    ...
    def layerbn(inputdata, is_training, name):
        ...
        if (data_format == 'NHWC'):
            ...
            return tf.layers.batch_normalization(inputs=inputdata, training=is_training, name=name, axis=1)
    ...
    def dilation_conv(input_tensor, k_size, out_dims, rate, padding='SAME',
        ...
        with tf.variable_scope(name):
            if (data_format == 'NCHW'):
                trans_input = tf.transpose(input_tensor, [0, 2, 3, 1])
            else:
                trans_input = input_tensor

            in_shape = trans_input.get_shape().as_list()
            ...
            if (data_format == 'NCHW'):
                final_ret = tf.transpose(ret, [0, 3, 1, 2])
            else:
                final_ret = ret
        ...
        def f2():
            ...
            with tf.variable_scope(name):
                if (data_format == 'NHWC'):
                    ...
                    num_feature_maps = [tf.shape(input_tensor)[0], tf.shape(input_tensor)[1]]
    ...
    def BNReluConv(net, num_filters, name, is_training, k=3, rate=1, first=False, concat=None):
        ...
        with tf.variable_scope(name):
            ...
            if not first:
                # TODO try Relu -> BN
                net = tf.layers.batch_normalization(net, training=is_training, name=name)
                net = tf.nn.relu(net)
                relu = net
            if concat is not None:
                net = tf.concat([net, concat], maps_dim)
                print('c ', net)
            net = tf.nn.conv2d(net, num_filters, kernel_size=k, rate=rate)
        return net